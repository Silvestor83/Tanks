    ...
    public class LogService
    {
        ...
        public LogService()
        {
            ...
            loggerFactory = UnityLoggerFactory.Create(builder =>
            {
                ...
                builder.AddZLoggerRollingFile(fileNameSelector: (dt, x) => $"logs/{dt.ToLocalTime():yyyy-MM-dd}_{x:000}.log",
                    timestampPattern: x => x.ToLocalTime().Date,
                    rollSizeKB: 1024,
                    configure: o =>
                    {
                        o.PrefixFormatter = (writer, info) => ZString.Utf8Format(writer, "[{0}][{1}]", info.LogLevel, info.Timestamp.DateTime.ToLocalTime());
                    });
            });
        ...
        public Microsoft.Extensions.Logging.ILogger Loggger => globalLogger;

        public ILogger<T> GetLogger<T>() where T : class => loggerFactory.CreateLogger<T>();
        public Microsoft.Extensions.Logging.ILogger GetLogger(string categoryName) => loggerFactory.CreateLogger(categoryName);
    }
}
